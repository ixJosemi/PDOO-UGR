    ...
    private void manageDiscardTreasures (boolean visible, Player aPlayer) {
        ...
        do { // Se descartan tesoros hasta que se vuelve al menú anterior
            ...
            if (option > Command.GoBack.menu) {
                ...
                if(visible) 
                    game.discardVisibleTreasure(new ArrayList (Arrays.asList(aPlayer.getVisibleTreasures().get(option))));
                else 
                    game.discardHiddenTreasure(new ArrayList (Arrays.asList(aPlayer.getHiddenTreasures().get(option)))); 
            }
